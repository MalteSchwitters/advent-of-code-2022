...
fn solve_part_2() -> String {
    ...
    for op in operations {
        ...
        // stack = &stacks.get_mut(op.to as usize).unwrap();
        // dbg!(&op);
        // dbg!(&stacks);
        // break;
        // 3, 1 => 3
        ...
        // 1.3 => 3.5
        // for i in 0..op.count {
        //     let stack = &stacks.get_mut(op.from as usize).unwrap();
        //     let items = stack.splice([0..op.count], []);
        //     let source_index = i;
        //     let target_index = op.count - i;

        //     let item = &stacks.get_mut(op.from as usize).unwrap().pop();
        //     let _ = &stacks.get_mut(op.to as usize).unwrap().push(item.unwrap());
        // }
    }
